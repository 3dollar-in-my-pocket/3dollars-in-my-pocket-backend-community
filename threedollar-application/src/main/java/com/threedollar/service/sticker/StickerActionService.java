...
public class StickerActionService {
    ...
    @Transactional
    public void upsertSticker(String workspaceId, AddStickerActionRequest request, StickerGroup stickerGroup, Set<Long> stickerList) {
        ...
        if (stickerAction != null) {
            stickerCountRepository.decrBulkByCount(workspaceId, stickerGroup, stickerAction.getTargetId(), stickerAction.getStickerIds());
    ...
    public List<TargetStickerAction> getStickerActionResponse(String workspaceId, StickerGroup stickerGroup,
                                                              @Nullable String accountId,
                                                              Set<String> targetIds,
                                                              List<Sticker> stickers) {

        Map<StickerActionCountKey, Long> stickerCountKeyLongMap = getStickerCountKey(workspaceId, stickerGroup, targetIds, stickers);

        Map<String, StickerAction> targetIdActedByMe = getTargetIdActedByMe(workspaceId, targetIds, accountId, stickerGroup);
        ...
        return targetIds.stream()
            .map(targetId -> {
                List<StickerInfoDetail> stickerInfoDetails = stickers.stream().map(
                    ...
                    sticker -> {
                        long count = stickerCountKeyLongMap.getOrDefault(StickerActionCountKey.of(workspaceId, stickerGroup, targetId, sticker.getId()), 0L);
    ...
    private Map<StickerActionCountKey, Long> getStickerCountKey(String workspaceId, StickerGroup stickerGroup,
                                                                Set<String> targetIds,
                                                                List<Sticker> stickers) {
        List<StickerActionCountKey> stickerCountKeys = targetIds.stream()
            .flatMap(targetId -> stickers.stream()
                .map(sticker -> StickerActionCountKey.of(workspaceId, stickerGroup, targetId, sticker.getId()))
            ).collect(Collectors.toList());
        return stickerCountRepository.getStickerCountMap(stickerCountKeys);

    }

    private Map<String, StickerAction> getTargetIdActedByMe(String workspaceId, Set<String> targetIds,
                                                            @Nullable String accountId,
                                                            StickerGroup stickerGroup) {
        if (StringUtils.isBlank(accountId)) {
            return Collections.emptyMap();
        }
        List<StickerAction> stickerActions = stickerActionRepository.getStickerActionByMe(workspaceId, accountId, targetIds, stickerGroup);
